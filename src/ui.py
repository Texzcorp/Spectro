...
class WallpaperGUI:
    def __init__(self, master):
        ...
        self.adult_filter_value = tk.BooleanVar(value=True)
        ...
        self.title_label.grid(row=0, column=0, columnspan=6, padx=10, pady=20, sticky="s")
        ...
        self.theme_text.grid(row=2, column=0, columnspan=6, padx=10, pady=10, sticky="nsew")
        master.grid_rowconfigure(2, weight=1)  # Make row expandable
        master.grid_columnconfigure(5, weight=1)  # Make column expandable
        ...
        self.save_button.grid(row=3, column=0, columnspan=2, padx=10, pady=10, sticky="w")
        ...
        self.num_label.grid(row=5, column=0, columnspan=2, padx=10, pady=10, sticky="e")
        
        self.num_var = tk.StringVar()
        self.num_spinbox = ttk.Spinbox(master, from_=1, to=100, textvariable=self.num_var, style="TSpinbox", font=self.custom_font)
        self.num_spinbox.grid(row=5, column=2, columnspan=4, padx=10, pady=10, sticky="ew")
        ...
        self.current_theme_label.grid(row=7, column=0, columnspan=6, padx=10, pady=20)
        ...
        self.download_button.grid(row=8, column=0, columnspan=6, padx=10, pady=10)
        ...
        self.message_label.grid(row=10, column=0, columnspan=6, padx=10, pady=10)
        ...
        self.autostart_checkbox.grid(row=3, column=5, padx=10, pady=10, sticky="w")

        self.progress_bar = ttk.Progressbar(master, mode='determinate', style="TProgressbar")
        self.progress_bar.grid(row=9, column=0, columnspan=6, padx=10, pady=10, sticky="we")
        self.progress_bar.grid_forget()

        # Dans la méthode __init__() de la classe WallpaperGUI
        self.suffix_label = ttk.Label(master, text="Theme Suffix:", foreground=self.text_color, background=self.background_color, style="Text.TLabel")
        self.suffix_label.grid(row=4, column=0, columnspan=2, padx=10, pady=1, sticky="e")

        self.suffix_entry = ttk.Entry(master, style="TEntry", font=self.custom_font)
        self.suffix_entry.grid(row=4, column=2, columnspan=4, padx=10, pady=1, sticky="ew")

        self.adult_filter_checkbox = ttk.Checkbutton(master, text="Keep wallpapers Safe For Work", variable=self.adult_filter_value, command=self.save_adult_filter_state, style="TCheckbutton")
        self.adult_filter_checkbox.grid(row=6, column=5, padx=10, pady=10, sticky="w")
    ...
    def configure_styles(self):
        ...
        style.configure("TEntry", foreground=self.text_color_button, fieldbackground=self.button_color, insertcolor=self.text_color, borderwidth=0, font=self.custom_font)
    ...
    def save_adult_filter_state(self):
        # Appeler save_datasave() avec l'état actuel de la case à cocher
    ...
    def save_themes(self):
        ...
        with open(os.path.join("data","themes.txt"), "w") as file:
            file.write("\n".join(themes))
    ...
    def download_async(self):
        ...
        isSFW = self.adult_filter_value.get()
        if themes:
            clear_wallpapers("wallpapers")
            self.save_datasave(self.currenttheme)
            random_theme = random.choice(themes)
            full_theme = f"{random_theme} {suffix}" if suffix else random_theme  # Ajouter le suffixe si présent
            self.progress_queue.put(0)  # Mettre à jour la file d'attente avec l'avancement du téléchargement
            download_images([full_theme], num_wallpapers, self.progress_queue, isSFW)
        ...
        try:
            with open(os.path.join("data", "datasave.txt"), 'r') as file:
                ...
                for line in lines:
                    ...
                    if line.startswith("themesuffix="):  # Charger le suffixe
                        suffix = line.split("=")[1].strip()
                        self.suffix_entry.delete(0, tk.END)  # Supprimer le texte existant
                        self.suffix_entry.insert(0, suffix)  # Mettre à jour le champ de saisie
                    if line.startswith("adult_filter="):
                        self.adult_filter_value.set(line.split("=")[1].strip() == "True") 
        except FileNotFoundError:
            pass
    ...
    def save_datasave(self, theme):
        ...
        suffix = self.suffix_entry.get()
        adult_filter = self.adult_filter_value.get()
        with open(os.path.join("data", "datasave.txt"), "w") as file:
            ...
            file.write(f"themesuffix={suffix}\n")
            file.write(f"adult_filter={adult_filter}\n") 
